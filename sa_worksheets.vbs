...
Worksheets(next_ws).Cells(1, 12).Value = "Yearly Change"
Worksheets(next_ws).Cells(1, 13).Value = "Percent Change"
Worksheets(next_ws).Cells(1, 14).Value = "Total Stock Volume"
summary_row = 2
total_stock = 0

'get the value for open in the second row of the new worksheet
open_pts = Worksheets(next_ws).Cells(2, 3).Value2

'find last row in worksheet
last_row = Worksheets(next_ws).Cells(Rows.Count, 1).End(xlUp).Row
    ...
    If Worksheets(next_ws).Cells(next_row + 1, 1).Value <> Worksheets(next_ws).Cells(next_row, 1).Value Then
  
        'add the last row of that stock volume to the total stock
        total_stock = total_stock + Worksheets(next_ws).Cells(next_row, 7).Value
        
        'get the current ticker symbol and put it in the summary table
        ticker = Worksheets(next_ws).Cells(next_row, 1).Value
        Worksheets(next_ws).Range("K" & summary_row).Value = ticker
        
        'put the total ticker stock in the summary table
        Worksheets(next_ws).Range("N" & summary_row).Value = total_stock
        'reset total volume to 0 when the ticker symbol changes
        total_stock = 0
      
       'get the close of the last row and put it in close_pts
        close_pts = Worksheets(next_ws).Cells(next_row, 6).Value

        'calculate the yearly change and put it in the summary table
        change = close_pts - open_pts
        Worksheets(next_ws).Range("L" & summary_row).Value = change
            If change < 0 Then
                'format neg change vaules to red
                Worksheets(next_ws).Range("L" & summary_row).Interior.ColorIndex = 3
            Else
                'format pos change values to green
                Worksheets(next_ws).Range("L" & summary_row).Interior.ColorIndex = 4
            End If
        
        'calculate percent change and put it in the sumary table
        pct_change = Round((change / open_pts) * 100, 2)
        Worksheets(next_ws).Range("M" & summary_row).Value = pct_change & "%"
        
        'get the open value of the first row in the next ticker symbol
        open_pts = Worksheets(next_ws).Cells(next_row + 1, 3).Value
